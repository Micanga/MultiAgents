...
def do_m_agent_move(sim, move):
	# 1. Initializing the move sim params
    get_reward = 0
    ...
    if move == 'L':
        ...
        load_item, (item_position_x, item_position_y) = is_agent_face_to_item(tmp_m_agent,sim)
        if load_item:
            destinantion_item_index = find_item_by_location(item_position_x, item_position_y.sim)
...
def evaluate_a_agent_action(a_agent, sim):
	...
	loc8ation = a_agent.position  # Location of main agent
	...
	# 6. Else we need to define the best policy
		...
		destination_index = sim.find_item_by_location(x_destination, y_destination,sim)
...
def destination_loaded_by_other_agents(agent,sim):
	...
	if destination_index != -1:
		item_loaded = items[destination_index].loaded
...
def update_the_map(sim):
    ...
    if sim.main_agent is not None:
        ...
        sim.the_map[m_agent_y][m_agent_x] = 9
